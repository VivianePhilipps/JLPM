...
For the continuous case, a transformation $H_k$ is estimated for outcome $y_k$ ($k \in 1,\dots,K$) :

\[
  H_k(y_k(t_{ijk}), \eta_k) = X_i(t_{ijk})\beta + Z_i(t_{ijk})u_i + \varepsilon_{ijk} 
\]
...
\[
  \mathbb{P}( y_k(t_{ijk}) = m) \Leftrightarrow \mathbb{P}(\eta_{k, m-1} <  X_i(t_{ijk})\beta + Z_i(t_{ijk})u_i + \varepsilon_{ijk} < \eta_{k, m})
\]

where  $X_i(t_{ijk})$ and $Z_i(t_{ijk})$ are vectors of covariates measured at time $t_{ijk}$ for subject $i$, $\beta$ is the vector of fixed effects, $u_i \sim \mathcal{N}(0,B)$ are the random effects,  $\varepsilon_{ijk} \sim \mathcal{N}(0,\sigma_k^2)$ the measurement errors, $\eta_k$ are the parameters of the link function $H_k$ or the thresholds associated to the outcome $y_k$.
...
\[
  \alpha_i(t) = \alpha_0(t, \omega) \exp(\tilde{X}_i \gamma + \delta u_i)
\]
...
\[
  \alpha_i(t) = \alpha_0(t, \omega) \exp(\tilde{X}_i \gamma + \delta (X_i(t)\beta + Z_i(t)u_i))
\]
...
\]
  ...
  with $\alpha_0(t, \omega)$ the baseline hazard function at time $t$, parameterized with $\omega$, $\tilde{X}_i$ a vector of time-fixed covariates, $\gamma$ the fixed effects, and $\delta$ the association parameter.
